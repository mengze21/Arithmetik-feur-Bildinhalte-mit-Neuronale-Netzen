...
from .support_lib import Breakout
...
class Edge:
    ...
    def trace(self, edge_list, initial_edge):
        ...
        if (abs(self.startpoint.coords[0]
                  -self.endpoint.coords[0])
              + abs(self.startpoint.coords[1]
                      -self.endpoint.coords[1])) < 1e-8:
...
class Straight(Edge):
    ...
    def _get_crossings(self):
        """Generates a transition list between pixels for
        rasterization."""
...
class Rectangle(Shape):
    ...
    def __init__(self, pos_x, pos_y, pos_z, len_1, len_2, len_3,
                   angle, beta, gamma, additive):
    ...
    def _el_crossings(self):
        ...
        def _cross(current, params, center):
            ...
            try:
                ...
                theta.append(2 * math.atan((radix-params[1])
                                           / (params[0]+current-center[0])))
                if radix > 0.0:
                # If the root turns out to be exactly zero, there is no
                # reason to return the same solution twice.  Therefore
                # the second value is only attached if that is not the
                # case.
                    theta.append(2 * math.atan((-radix-params[1])
                                               / (params[0]+current-center[0])))
...
class FieldScope:
    ...
    def _arc_cross(self, arc1, arc2):
        ...
        # with cases (3) and (4)
            ...
            path_taken = 'diff does not contain y and xy terms'
    ...
    def register_shape(self, new_shape):
        ...
        intensities per object.  To reflect this, this method is
        changed such that object are no longer fused, but still
        intersected.
